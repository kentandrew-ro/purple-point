...
async function ensureDbInitialized() {
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS dental_records (
      ...
      treatment_plan_notes VARCHAR(255),
      clinical_notes TEXT,
      ...
      CONSTRAINT fk_dental_records_patient
        FOREIGN KEY (patient_id) REFERENCES patients(patient_id)
        ON DELETE CASCADE,
      CONSTRAINT fk_dental_records_appointment
        FOREIGN KEY (appointment_id) REFERENCES appointments(appointment_id)
        ON DELETE SET NULL,
      CONSTRAINT fk_dental_records_dentist
        FOREIGN KEY (dentist_id) REFERENCES dentist(dentist_id)
        ON DELETE SET NULL,
      CONSTRAINT fk_dental_records_staff
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS patient_records (
      ...
      CONSTRAINT fk_patient_records_patient
        FOREIGN KEY (patient_id) REFERENCES patients(patient_id)
        ON DELETE CASCADE
    ) ENGINE=InnoDB;
  `);
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS patient_vitals (
      ...
      CONSTRAINT fk_patient_vitals_patient
        FOREIGN KEY (patient_id) REFERENCES patients(patient_id)
        ON DELETE CASCADE,
      CONSTRAINT fk_patient_vitals_record
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS tooth_chart (
      ...
      condition_status ENUM('healthy', 'treated', 'needs_attention', 'extracted') NOT NULL DEFAULT 'healthy',
      updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
      UNIQUE KEY uq_patient_tooth (patient_id, tooth_number),
      CONSTRAINT fk_tooth_chart_patient
        FOREIGN KEY (patient_id) REFERENCES patients(patient_id)
        ON DELETE CASCADE
    ) ENGINE=InnoDB;
  `);
